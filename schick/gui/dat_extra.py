...
class SchickDatTlkContent(ttk.Frame):
    def __init__(self, master, schick_reader, fname):
        ...
        self.text = Text(self, padx=10, pady=10)
        ...
        self.lbox.grid(column=1, row=0, rowspan=2, pady=5, sticky=(N,W,E))
        self.state_select.grid(column=0, row=2, columnspan=2, pady=5, sticky=(N,E,S,W))
        self.text.grid(column=0, row=3, columnspan=2, pady=5, sticky=(N,E,S,W))
    ...
    def load_in_state(self, idx):
        ...
        if state['txt_id'] != -1:
            ...
            if state['opt1'] != 0 or state['opt2'] != 0 or state['opt3'] != 0:
                if not self.random: output += "\n"
                for n,opt in enumerate([state['opt1'],state['opt2'],state['opt3']]):
                    if opt == 0: continue
                    for r in range(4 if self.random else 1):
                        opt_txt = self.texts[self.informer['txt_offset'] + opt + r]
                        if opt_txt == "": continue
                        output += "{:d}: {}\n".format(n, opt_txt)
